...
class App(tk.Tk):
    ...
    def _build_header(self):
        ...
        header.pack(fill="x")
        tk.Label(header, text="Bloons TD Battles",
                 bg=ACCENT2, fg="white",
    ...
    def _build_lobbies_tab(self):
        ...
        tk.Label(inner, text="Active Game Lobbies", bg=SURFACE, fg=TEXT,
        ...
        self.btn_refresh_lobbies = tk.Button(btn_frame, text="🔄 Refresh", command=self._on_refresh_lobbies,
                                             state="disabled", bg=SURFACE, fg=TEXT, relief="flat")
        self.btn_refresh_lobbies.pack(side="left", padx=5)

        self.btn_create_lobby = tk.Button(btn_frame, text="➕ Host Match", command=self._on_create_lobby,
                                          state="disabled", bg=ACCENT, fg="white", relief="flat")
        self.btn_create_lobby.pack(side="left", padx=5)

        self.btn_join_lobby = tk.Button(btn_frame, text="⚔️ Join Match", command=self._on_join_lobby,
                                        state="disabled", bg=SUCCESS, fg="#1e1e2e", relief="flat")
        self.btn_join_lobby.pack(side="left", padx=5)
    ...
    def _on_signup(self):
        ...
        self.log_box.see("end")
        msg = (f"SIGNUP~{username}~{self.su_pass.get()}"
               f"~{self.su_email.get()}")
    ...
    def _enable_lobby_controls(self):
        self.btn_refresh_lobbies.config(state="normal")
        self.btn_create_lobby.config(state="normal")
        self.btn_join_lobby.config(state="normal")
        self._on_refresh_lobbies()

    def _on_refresh_lobbies(self):
        if self.cli_sock:
            send_with_size(self.cli_sock, b"GET_LOBBIES~", encryption_key)

    def _on_create_lobby(self):
        if self.cli_sock and self.logged_in_username:
            send_with_size(self.cli_sock, b"CREATE_LOBBY~", encryption_key)
        ...
        while True:
            try:
                ...
                # Successful login → publish our public key
                    ...
                    self.after(0, self._enable_lobby_controls)
                ...
                # Receive a private (E2E encrypted) message
                    ...
                    _, opponent, role = data.split("~")
                    self.log_box.insert("end", f"Match found against {opponent}! Launching...")

                    # Hide Tkinter window safely
                    self.withdraw()

                    # Launch game, passing the active socket, key, and side assignment
                    from Game import launch_multiplayer_game
                    launch_multiplayer_game(self.cli_sock, encryption_key, role, opponent)

                    # Bring launcher back after game ends
                    self.deiconify()

                elif data.startswith("LOBBY_LIST~"):
                    rooms = [r for r in data.split("~")[1:] if r]
                    self.after(0, self._update_lobby_ui_list, rooms)
                    self.log_box.insert("end",
                                        f"[🎮] Received active lobbies list: {', '.join(rooms) if rooms else 'None'}")
